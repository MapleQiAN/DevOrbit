# GitHub OAuth 快速参考指南

## 🎯 快速开始

### 1. 获取 GitHub OAuth 凭证

访问 https://github.com/settings/developers → "New OAuth App"

**填写信息**:
- Application name: `DevOrbit`
- Homepage URL: `http://localhost:3000`
- Authorization callback URL: `http://localhost:3000/auth/github/callback`

**获取凭证**:
- Client ID
- Client Secret

### 2. 配置环境变量

编辑 `backend/.env`:

```env
GITHUB_CLIENT_ID=your_client_id_here
GITHUB_CLIENT_SECRET=your_client_secret_here
GITHUB_REDIRECT_URI=http://localhost:3000/auth/github/callback
```

### 3. 启动后端

```bash
cd backend
python -m uvicorn app.main:app --reload
```

### 4. 测试 OAuth 流程

#### 方式 A: 使用 API 文档（推荐）

1. 打开 http://localhost:8000/docs
2. 找到 `/auth/github/login` 端点
3. 点击 "Try it out" → "Execute"
4. 复制返回的 `authorization_url`
5. 在浏览器中打开此 URL
6. 授权后，GitHub 会重定向到回调 URL
7. 手动访问 `/auth/github/callback?code=<code>` 完成流程

#### 方式 B: 使用测试脚本

```bash
cd backend
python test_oauth.py
```

#### 方式 C: 使用 curl

```bash
# 1. 获取授权 URL
curl http://localhost:8000/auth/github/login

# 2. 在浏览器中打开返回的 authorization_url
# 3. 授权后，GitHub 重定向到回调 URL，提取 code 参数
# 4. 调用回调端点
curl "http://localhost:8000/auth/github/callback?code=<code>"
```

---

## 📋 API 端点详解

### GET /auth/github/login

**功能**: 获取 GitHub OAuth 授权 URL

**请求**:
```bash
curl http://localhost:8000/auth/github/login
```

**响应**:
```json
{
  "authorization_url": "https://github.com/login/oauth/authorize?client_id=xxx&..."
}
```

**说明**:
- 前端调用此端点
- 获取授权 URL
- 跳转到 GitHub 授权页面

---

### GET /auth/github/callback

**功能**: 处理 OAuth 回调，交换 token，创建用户，签发 JWT

**请求参数**:
- `code` (必需): GitHub OAuth 授权码
- `state` (可选): CSRF 防护状态码

**请求示例**:
```bash
curl "http://localhost:8000/auth/github/callback?code=abc123"
```

**响应**:
```json
{
  "access_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "token_type": "bearer",
  "user": {
    "id": 1,
    "github_id": 12345678,
    "github_login": "octocat",
    "avatar_url": "https://avatars.githubusercontent.com/u/1?v=4",
    "created_at": "2025-12-09T11:50:21.953000",
    "updated_at": "2025-12-09T11:50:21.953000"
  }
}
```

**说明**:
- 返回的 `access_token` 是 JWT token
- 前端应将其存储到 localStorage
- 后续请求时，在 Authorization header 中使用：`Authorization: Bearer <token>`

---

## 🔐 使用 JWT Token

### 存储 Token

```javascript
// 前端 (JavaScript)
const response = await fetch('http://localhost:8000/auth/github/callback?code=xxx');
const data = await response.json();
localStorage.setItem('access_token', data.access_token);
```

### 发送 Token

```javascript
// 在后续请求中使用 token
const token = localStorage.getItem('access_token');
const response = await fetch('http://localhost:8000/github/sync', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${token}`,
    'Content-Type': 'application/json',
  },
});
```

### 后端验证 Token

```python
from app.api.deps import get_current_user
from app.models import User

@router.get("/protected")
async def protected_endpoint(current_user: User = Depends(get_current_user)):
    return {"user_id": current_user.id}
```

---

## 🔄 完整 OAuth 流程图

```
┌─────────────────────────────────────────────────────────────┐
│                        前端 (Vue 3)                         │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
                    1. 点击「GitHub 登录」按钮
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 调用 GET /auth/github/login │
                  └─────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │  获取 authorization_url     │
                  └─────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 跳转到 GitHub 授权页面      │
                  └─────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                     GitHub 授权页面                         │
│                   用户输入用户名和密码                       │
│                        用户授权                             │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ GitHub 重定向到回调 URL     │
                  │ ?code=xxx&state=yyy         │
                  └─────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 前端提取 code 参数          │
                  └─────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 调用 /auth/github/callback  │
                  │ ?code=xxx                   │
                  └─────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                     后端 (FastAPI)                          │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. 使用 code 向 GitHub 交换 access_token                   │
│     POST https://github.com/login/oauth/access_token       │
│                                                             │
│  2. 使用 access_token 获取用户信息                          │
│     GET https://api.github.com/user                        │
│                                                             │
│  3. 在数据库中创建或更新用户                                │
│     INSERT/UPDATE users                                    │
│                                                             │
│  4. 签发 JWT token                                          │
│     jwt.encode({"sub": user_id, "exp": ...})               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 返回 JWT token 和用户信息   │
                  └─────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 前端存储 token 到 localStorage
                  └─────────────────────────────┘
                              │
                              ▼
                  ┌─────────────────────────────┐
                  │ 跳转到仪表盘页面            │
                  └─────────────────────────────┘
```

---

## 🐛 常见问题

### Q1: 获取 OAuth 凭证时出现 "Invalid redirect URI"

**原因**: 回调 URL 与 GitHub 应用中配置的不匹配

**解决**:
1. 访问 https://github.com/settings/developers
2. 编辑应用设置
3. 确保 "Authorization callback URL" 与 `.env` 中的 `GITHUB_REDIRECT_URI` 一致

### Q2: 调用 /auth/github/callback 时返回 500 错误

**原因**: 可能是 GitHub OAuth 凭证配置不正确

**解决**:
1. 检查 `.env` 文件中的 `GITHUB_CLIENT_ID` 和 `GITHUB_CLIENT_SECRET`
2. 确保没有多余的空格或引号
3. 检查后端日志中的详细错误信息

### Q3: JWT token 过期了怎么办？

**解决**:
1. 前端检测到 401 错误
2. 清除本地存储的 token
3. 重新调用 `/auth/github/login` 进行登录

### Q4: 如何修改 JWT 过期时间？

编辑 `.env`:

```env
JWT_EXPIRE_MINUTES=10080  # 7 days
```

或在代码中指定：

```python
from datetime import timedelta
token = create_access_token(
    subject=str(user.id),
    expires_delta=timedelta(days=30)
)
```

---

## 🔒 安全建议

### 生产环境配置

1. **更改 JWT 密钥**:
   ```env
   JWT_SECRET_KEY=your_very_long_random_secret_key_at_least_32_chars
   ```

2. **使用 HTTPS**:
   - 所有 OAuth 流程必须使用 HTTPS
   - 配置 `GITHUB_REDIRECT_URI` 为 HTTPS URL

3. **CORS 配置**:
   ```env
   CORS_ORIGINS=["https://yourdomain.com"]
   ```

4. **保护 GitHub Token**:
   - 不要在前端暴露 GitHub access token
   - 只在后端存储和使用

5. **使用 CSRF 防护**:
   - 在 OAuth 流程中使用 `state` 参数
   - 验证回调时的 state 值

---

## 📚 相关资源

- [GitHub OAuth 文档](https://docs.github.com/en/developers/apps/building-oauth-apps)
- [GitHub API 文档](https://docs.github.com/en/rest)
- [JWT 介绍](https://jwt.io/introduction)
- [FastAPI 安全文档](https://fastapi.tiangolo.com/tutorial/security/)

---

## 🚀 下一步

完成 OAuth 认证后，可以进入 Stage 3：

- 实现 GitHub 数据同步
- 实现每日统计查询
- 实现前端登录流程

---

**最后更新**: 2025-12-09  
**版本**: 0.1.0

